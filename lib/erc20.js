...
Erc20Controller.prototype.getContractTransactions = function(req, res) {
    ...
    var self = this,
        ...
        addresses = (req.query.addresses ? req.query.addresses.split(",") : []),
...
Erc20Controller.prototype.findBalancesByTransferAddress = function(req, res) {
    ...
    var addresses = balanceAddress.split(',');

    return async.waterfall([function (callback) {

        return self.erc20BalanceRepository.fetchBalancesByAddresses(addresses, function (err, balances) {
        ...
        var contractsAddresses = dataFlow.balances.map(function (balance) {
          return balance.contract_address;
        }).filter(function (contract_address, i, array) {
          return array.indexOf(contract_address) === i;
        });
        ...
        var contractsHash = dataFlow.contracts.reduce(function (a, contract) {
          a[contract.contract_address] = contract;
          return a;
        }, {});

        var result = dataFlow.balances.map(function (balance) {
          return {
            amount: balance.amount,
            address: balance.address,
            address_eth: balance.address_eth,
