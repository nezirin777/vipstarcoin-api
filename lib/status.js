...
StatusController.prototype.getInfo = function(callback) {
  this.node.services.vipstarcoind.getInfo(function(err, result) {
    ...
    var info = {
      ...
      proxy: result.proxy,
      difficulty: {
        'proof-of-stake': result.difficulty,
        'proof-of-work': result.difficulty // 現在の全体の難易度を両方に割り当てます
      },
...
StatusController.prototype.getDifficulty = function (callback) {
  this.node.services.vipstarcoind.getInfo(function (err, info) {
    ...
    // オブジェクトの場合は proof-of-stake の数値を返し、そうでなければそのまま返します
    var diffVal = (info.difficulty && typeof info.difficulty === 'object')
      ? info.difficulty['proof-of-stake']
      : info.difficulty;
